...
# Data url
URL_DATA = (
    "https://www.kaggle.com/api/v1/datasets/download/sasakitetsuya/machine-rul-data"
)
# Local tracking URI
...
def read_dataframe(url: str) -> pd.DataFrame:
    ...
    dataframe.
    ---
    url: url address of the zip data file
        str
    """
    # Download the data
...
def train_model(train_dicts, y_train, val_dicts, y_val):
    ...
    with mlflow.start_run() as run_exp:
        ...
        params = dict(
            max_depth=20, n_estimators=100, min_samples_leaf=10, random_state=0
        )
    ...
    mlflow.register_model(
        model_uri=f"runs:/{run_exp.info.run_id}/models", name=model_name
    )
...
def monitoring(df_val, df_test, numerical, categorical):
    ...
    data_definition = DataDefinition(
        numerical_columns=numerical + ["remaining_useful_life"],
    ...
    # Run the report
    snapshot = report.run(reference_data=val_dataset, current_data=test_dataset)
...
def run(url: str):
    ...
    machine_df = read_dataframe(url)
    # Prepare the data
    machine_df = prepare_dataframe(machine_df)

    # Features lists
    numerical = (
        machine_df.drop(columns="remaining_useful_life")
    ...
    df_full_train, df_test = train_test_split(
        machine_df, test_size=0.2, random_state=42
    )
